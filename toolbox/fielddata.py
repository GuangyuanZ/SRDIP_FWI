...
from tools import array2su, get_offset, get_su_parameter, loadsu, savesu,  su2array


def load_field_data(simu, data_list, x_beg, x_end, drift=0.0):
    ...
    dt = simu.model.dt
    nt = simu.model.nt
    comp = simu.cable.component
    
    print('Field data: loading data and adjusting to current acquisition')

    pool = Pool(nproc)
    receiver_range = [pool.apply_async(load_field_data_serial, (homepath, data_list[isrc], isrc, x_beg, x_end, dt, nt, comp, drift)) for isrc in range(srcn)]
    pool.close()
...
def load_field_data_serial(homepath, loadfile, isrc, x_beg, x_end, dt, nt, comp, drift):

        # set path
        savefile = homepath + 'data/obs/src%d_sg_%s.su' %(isrc + 1,comp)

        # load data
        trace = loadsu(loadfile)
        _, _, dt_data = get_su_parameter(trace)
        x_beg *= trace[0].stats.su.trace_header.scalar_to_be_applied_to_all_coordinates
        x_end *= trace[0].stats.su.trace_header.scalar_to_be_applied_to_all_coordinates
        ...
        for tr in trace:
            ...
            tr.trim(starttime=t0+drift, endtime=t0 + dt*(nt-1)+drift, pad=True, nearest_sample=True, fill_value=0.0)
...
# def load_farfield_signature(filename,dt_wavelet,dt,starttime_wavlet,starttime,nt, f_low,f_high):
def load_farfield_signature(wavelet,dt_wavelet,dt,starttime_wavelet,starttime,nt, f_low,f_high):
    # wavelet = np.loadtxt(filename).flatten()
    nt_wavelet = round((nt-1)*(dt/dt_wavelet)+1)
    nt_ahead = round((starttime_wavelet-starttime)/dt_wavelet)
    wavelet = np.concatenate((np.zeros(nt_ahead),wavelet,np.zeros(nt_wavelet-len(wavelet)-nt_ahead)))
    fs = 1.0 / dt_wavelet
    nyq = 0.5 * fs
    low = f_low / nyq
    high = f_high / nyq
    b, a = butter(3, [low, high], btype='band')
    wavelet_filt = filtfilt(b, a, wavelet)
    return wavelet_filt[::round(dt/dt_wavelet)]