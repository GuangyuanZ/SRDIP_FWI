...
from misfit import adjoint_source,regular_StepLR
from postprocess import grad_precond, grad_precond_tv, grad_precond_tv1
from tools import cleandata, loadbinfloat32, savebinfloat32,array2vector
...
def adjoint_tv(simu, optim,inv_scheme,iter):
    ''' adjoint solver: fd2dmpi (2D acoustic)
    '''
    ...
    grad_norm = grad_precond_tv(simu, optim, grad, forw, back,iter)


    return grad_norm
    # return -array2vector(m)

def adjoint_tv1(simu, optim,inv_scheme,iter,px,py,bx,by):
    ''' adjoint solver: fd2dmpi (2D acoustic)
    '''
    ...
    grad_fwi = grad_precond_tv1(simu, optim, grad, forw, back,px,py,bx,by)



    # return grad_fwi + grad_norm
    return grad_fwi
    # return -array2vector(m)

def shrink(x, lambda_):
