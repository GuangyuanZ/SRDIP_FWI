...
from misfit import misfit, misfit_tv,misfit_tv1
from preprocess import process_workflow
from solver import adjoint, forward, adjoint_tv,adjoint_tv1
...
import time
from postprocess import shrink,grad_de
'''
         d_old            d_now    
...
def inversion(simu, optim, inv_model):
    ...
    juadge_thred = 0.000000003
    judge_num = 0
    v_tmp = np.zeros_like(inv_scheme['v_now'])

    inv_scheme['a1'] = np.zeros([simu.model.nx,simu.model.nz])
    inv_scheme['a2'] = np.zeros([simu.model.nx,simu.model.nz])
    inv_scheme['b1'] = np.zeros([simu.model.nx,simu.model.nz])
    inv_scheme['b2'] = np.zeros([simu.model.nx,simu.model.nz])
    lmbda_tmp = simu.model.lmbda 

    px = np.zeros_like(simu.model.vp)
    py = np.zeros_like(simu.model.vp)
    bx = np.zeros_like(simu.model.vp)
    by = np.zeros_like(simu.model.vp)
    ...
    while optim.iter < optim.maxiter:
        ...
        # plot_trace(simu, 'syn', simu_type='syn', suffix='', src_space=1, trace_space=1, scale = 0.8, color='b')
        # plot_trace(simu, 'syn-proc', simu_type='syn', suffix='_proc', src_space=1, trace_space=5, scale = 0.8, color='b')


        # evaluate the misfit
        if  optim.misfit_type.lower() in ['globalcorrelation_tv']:
            inv_scheme['f_now'] = misfit_tv(simu, optim.misfit_type,optim.iter)   
            inv_scheme['g_now'] = adjoint_tv(simu, optim,inv_scheme,optim.iter)   
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/a1_2.dat',inv_scheme['a1'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/a2_2.dat',inv_scheme['a2'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/b1_2.dat',inv_scheme['b1'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/b2_2.dat',inv_scheme['b2'])
            ...
            inv_scheme = backtrack_tv(simu, optim, inv_scheme,optim.iter)
            lmbda = simu.model.lmbda
            print('Previous misfit lmbda: ',lmbda)
            if optim.iter % simu.model.step_size == 0:
                lmbda = lmbda_tmp*np.power(simu.model.gamma, optim.iter // simu.model.step_size)
            simu.model.lmbda = lmbda
            print('Current misfit lmbda: ',lmbda)            
        
        elif  optim.misfit_type.lower() in ['globalcorrelation_tv1']:

            inv_scheme['f_now'] = misfit_tv1(simu, optim.misfit_type,optim.iter)   
            inv_scheme['g_now'] = adjoint_tv1(simu, optim,inv_scheme,optim.iter,px,py,bx,by)   
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/a1_2.dat',inv_scheme['a1'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/a2_2.dat',inv_scheme['a2'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/b1_2.dat',inv_scheme['b1'])
            # np.savetxt('/data/guangyuan/SWIT-1.0/examples/FWI_reg/b2_2.dat',inv_scheme['b2'])
            ...
            # search for the step length
            inv_scheme = backtrack_tv1(simu, optim, inv_scheme,optim.iter)
            mx, my = grad_de(simu.model.vp)
            px = shrink(mx + bx, simu.model.lmbda / simu.model.gamma)
            py = shrink(my + by, simu.model.lmbda / simu.model.gamma)
            bx = bx + (mx - px)
            by = by + (my - py)
            print('Current m:',simu.model.vp.max())
            print('Current px:',px.max())
            print('Current py:',py.max())
            print('Current bx:',bx.max())
            print('Current by:',by.max())
        else:
            inv_scheme['f_now'] = misfit(simu, optim.misfit_type)   
            inv_scheme['g_now'] = adjoint(simu, optim)
            # G_now = np.array(inv_scheme['g_now'])
        ...
        misfit_tmp = inv_scheme['f_now']
        inv_scheme = desicion_make(simu, optim, inv_scheme)

        print('Check work or not')
        print(inv_scheme['f_now'])
        print(inv_scheme['f_old'])
        print(np.mean(inv_scheme['v_now']))
        print(np.mean(v_tmp))
        if judge_num < 10000:
            if np.abs(inv_scheme['f_now'] -inv_scheme['f_old'])  <= np.abs(juadge_thred * inv_scheme['f_old']):
                judge_num += 1
            else:
                judge_num = 0
            v_tmp = inv_scheme['v_now']
        
        else:
            simu.model.vp = vector2array(v_tmp, simu.model.nx, simu.model.nz)
            inv_scheme['f_now'] = misfit_tmp
        print(judge_num)
    ...
    print('Inverse total time:',time.time()-start_time_init)
    print('\n-----------  iteration end  -----------\n')
...
def backtrack_tv(simu, optim, inv_scheme,itera):
    ''' backtrack linesearch
    '''
    # define parameters for line search
    dec = 0.5          # half the step
    ...
    while True:
        ...
        process_workflow(simu, optim, simu_type='syn')
        # misfit from try model
        f_try = misfit_tv(simu, misfit_type,itera)
...
def backtrack_tv1(simu, optim, inv_scheme,itera):
    ''' backtrack linesearch
    '''
    # define parameters for line search
    dec = 0.5          # half the step
    ...
    while True:
        ...
        process_workflow(simu, optim, simu_type='syn')
        # misfit from try model
        f_try = misfit_tv1(simu, misfit_type,itera)
...
def source_inversion(simu, inv_offset=10000):
    ...
    plot_wavelet(simu, stf1_now, 'stf_now', scale=1.0, color='k', plot_dx=5000, t_end = 1.0)
    plot_wavelet(simu, stf1_inv, 'stf_inv', scale=1.0, color='r', plot_dx=5000, t_end = 1.0)
