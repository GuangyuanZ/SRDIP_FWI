...
### Plot trace (waveform)
def plot_trace(simu, filename, simu_type='syn', suffix='', src_space=1, trace_space=1, scale=1.0, color='r', plot_dx=2000):
    ...
    for isrc in range(srcn):
        ...
        plot_trace_serial(datapath, figname, trace_space, scale, color, 
                        plot_dx,nt, dt, isrc, 'pressure', )
        time.sleep(0.001)
    pool.close()
    pool.join()
...
def plot_inv_scheme(simu, optim, inv_scheme):
    ...
    if optim.iter == 1 :
        plot_trace(simu, 'syn-proc-initial', simu_type = 'syn', suffix='_proc', src_space=1, trace_space=5, scale=0.8, color='r',plot_dx=2000000)
        ...
        plot_trace(simu, 'syn-proc-final', simu_type = 'syn', suffix='_proc', src_space=1, trace_space=5, scale=0.8, color='b',plot_dx=20000000)
    else:
        pass
